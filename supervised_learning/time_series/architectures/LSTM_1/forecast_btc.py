...
def build_rnn_model(input_shape, rnn_type='lstm'):
    ...
    # First RNN layer
    if rnn_type == 'lstm':
        model.add(LSTM(1, activation='relu', return_sequences=False, input_shape=input_shape))
    elif rnn_type == 'gru':
        model.add(GRU(1, activation='relu', return_sequences=False, input_shape=input_shape))
    elif rnn_type == 'simple_rnn':
        model.add(SimpleRNN(1, activation='relu', return_sequences=False, input_shape=input_shape))
    else:
